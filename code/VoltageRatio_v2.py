...
try:
    from PhidgetHelperFunction_v2 import *
except ImportError:
...
def onVoltageRatioChangeHandler(self, voltageRatio):
    ...
    count += 1
    volts[self.getChannel()] = voltageRatio
    if count % 4 == 0:
        count = 0
        print (volts)
...
for i in range(len(ch)):
    print("\nOpening and Waiting for Attachment...", i)
    
    try:
        ch[i].openWaitForAttachment(5000)
        ch[i].setDataInterval(250)
    except PhidgetException as e:
        PrintOpenErrorMessage(e, ch[i])
        raise EndProgramSignal("Program Terminated: Open Failed")
